...
const Finance = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${import.meta.env.VITE_API_BASE || "https://api.shivaamfarmsandresorts.com"}/api/stocks`)
      .then((res) => {
        ...
        return axios.get(
          `${import.meta.env.VITE_API_BASE || "https://api.shivaamfarmsandresorts.com"}/api/bookings`
        );
      })
      .then((res) => {
  ...
  const handleExportPDF = () => {
    ...
    // Sort by date ascending for PDF export (earliest first, latest last)
    const sortedForExport = [...transactionsForDisplay].sort(
      (a, b) => new Date(a.date) - new Date(b.date)
    );

    sortedForExport.forEach((t, i) => {
      const transactionData = [
        i + 1,
        t.source,
        t.description,
        t.date ? new Date(t.date).toLocaleDateString() : "",
        `Rs. ${(t.advancedAmount ?? 0).toLocaleString()}`,
        `Rs. ${(t.remainingAmount ?? 0).toLocaleString()}`,
        t.status ?? "-",
        t.receivedBy ?? "-",
        t.paymentMode ?? "-",
        t.paymentCategory ?? "-",
        `Rs. ${(Number(t.total_amount) || 0).toLocaleString()}`,
      ];
    ...
    tableRows.push([
      ...
      { content: `Rs. ${grandTotal.toLocaleString()}`, styles: { fontStyle: "bold" } },
    ]);
  ...
  const handleExportCSV = (transactions) => {
    ...
    // Sort by date ascending for CSV export (earliest first, latest last)
    const sortedForExport = [...transactions].sort(
      (a, b) => new Date(a.date) - new Date(b.date)
    );

    const exportData = sortedForExport.map((t, i) => ({
      "#": i + 1,
      Source: t.source,
